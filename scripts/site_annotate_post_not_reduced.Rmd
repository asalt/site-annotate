...
data_dir <- "/media/alex/Expansion/MSPC000668/data/processed/phos/fragpipe"
...
possible_id_cols <- c("ENSG", "ENST", "ENSP", "geneid", "symbol", "taxon", "fifteenmer", "position", "AA", "sitename")
.metadatas <- datas %>% purrr::map(~{
    common_cols <- intersect(colnames(.x), possible_id_cols)
  res = .x %>%
    distinct(!!!rlang::syms(common_cols)) %>% 
    mutate(sitename = stringr::str_c(symbol, stringr::str_c(!!modi_abbrev, AA, position), sep='_'))
})
.metadata <- bind_rows(.metadatas) %>% distinct()

.datas_reduced <- datas_reduced %>% purrr::map(~{
   common_cols <- intersect(colnames(.x), possible_id_cols)
  .x %>% left_join(.metadata) %>% dplyr::select(!!common_cols, everything())
})
...
isoform_table <- package_files %>% purrr::map(~read_tsv(.x, show_col_types = F)) %>% bind_rows()
...
condense_group <- function(df) {
  ...
  if (is.na(primarysel) && is.na(secondarysel)) {
    if (length(unique(df$ENSP)) == 1) {
      ...
      bestsel <- df[which.max(df$protein_length), "Protein"]
    }
  }
