...
generate_global_heatmaps <- function(gct_z, config, outdir, friendly_name, outname) {
...
generate_global_heatmap <- function(gct_z, cut_by, cluster_columns, config, outdir, friendly_name, outname) {
  ...
  outf <- heatmap_tools$prepare_heatmap_output_path(outdir,
    "heatmap",
    outname,
    dimensions = list(nrow=nrow(gct_z_subset@mat), ncol(gct_z_subset@mat)),
    cut_by = cut_by,
    cluster_rows = T,
    cluster_columns = cluster_columns
  )
...
generate_single_selection_heatmap <- function(subgct_z, cut_by, cluster_rows, cluster_columns, outdir, outname, file_name='', friendly_name = '', config) {
  ...
  # Draw and save the heatmap
  heatmap_tools$draw_and_save_heatmap(ht_draw_code, outf, subgct_z)
}
